...
# Model fitting. The rep is the block. For split-plot, we nest the whole-plot (krate) inside the rep (block); in SAS it would be the opposite. 
...
# summary output: krate:rep and rep are the random effects. Residual comes from the nrate. 



# In case we have P rate it would be a split-split-plot
spp_rcbd_mix_mod <- lmer(yiel_kgha ~ krate_kgha * nrate_kgha * prate_kgha + (1|rep/krate_kgha/nrate_kgha)) # we do not specify the smallest size (the residual)

# I can not randomize space and time. Time and space is linear (there is a connection), so have to analyze it as a repeated measure. For a split plot, all the whole plot and and split plot can be randomized. 
```
...
Also notice how block (i.e., rep in this case) does not appear in the ANOVA table above. That's because it only displays **fixed effects**.
...
randeff_blockkrate # 12 points because 3 krates and 4 reps
```
...
spp_rcbd_mix_means_all <- emmeans(spp_rcbd_mix_mod, 
                                  ~nrate_kgha:krate_kgha) # not neccessary to order
...
My note: for unbalanced data we can expect the SE's to be different. 
...
```{r k rate means}
spp_rcbd_mix_means_all <- emmeans(spp_rcbd_mix_mod, 
                                  ~nrate_kgha) # not neccessary to order
...
```{r n rate means }
spp_rcbd_mix_means_all <- emmeans(spp_rcbd_mix_mod, 
                                  ~ krate_kgha) # not neccessary to order
...
# Quiz questions
What is the main difference int he model function between a split plot in a RCBD vs a RCBD? 
- The whole-plot was nested within the block or rep. 
